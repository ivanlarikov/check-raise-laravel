...
if ($getAllToomsResult= $mysqli_new->query("SELECT id FROM `rooms`")) {
    ...
    while($room = $getAllToomsResult -> fetch_assoc()){
        ...
        while($tran=$translations-> fetch_assoc())
        {
            ...
            $sql="SELECT `meta_value` FROM `jtamvgr0h_postmeta` WHERE `post_id` = ".$tran['element_id']." AND `meta_key` = 'description_of_the_poker_room'";
            $description="";
        ...
        foreach($descriptions as $key=>$value){
            $sql="INSERT INTO `room_descriptions`(`room_id`, `language`, `description`) VALUES ('".$value['room_id']."','".$value['language']."','".$value['description']."')";
